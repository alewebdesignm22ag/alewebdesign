...
class Mail {
	...
	public $protocol = 'mail'; // tem smtp tambem
			...
			if ($this->parameter) {
				...
				mail($to, '=?UTF-8?B?' . base64_encode($this->subject) . '?=', $message, $header);
                    ...
                    //
                        ...
                        $mail->SMTPDebug = true;
                        $mail->Host     = $this->smtp_hostname; // SMTP server
                        ...
                        $result .= $mail->HeaderLine("Message-ID" , "<".md5(uniqid(time())).$this->smtp_hostname);
                        ...
                        $result .= $mail->HeaderLine("X-Sender" , $this->sender);
                        $result .= $mail->HeaderLine("X-AntiAbuse" , "This is a solicited email for - "."SITE"." mailing list.");
                        $result .= $mail->HeaderLine("X-AntiAbuse" , "Servername - {$this->smtp_hostname}");
                        $result .= $mail->HeaderLine("X-AntiAbuse" , $this->sender);
                        */
