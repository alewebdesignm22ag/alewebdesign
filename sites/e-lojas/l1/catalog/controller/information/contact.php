	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			unset($this->session->data['captcha']);
                        ...
                        $mail->SMTPDebug = true;
                        $mail->Host     = $this->config->get('smtp_hostname'); // SMTP server
                        $mail->SetFrom($this->config->get('smtp_username'),$config->get("config_owner"));
                        $mail->SMTPAuth = true;     // turn on SMTP authentication
                        $mail->SMTPDebug = 2;
                        $mail->SMTPSecure = 'tls';
                        $mail->Username = $this->config->get('smtp_username');  // SMTP username
                        $mail->Password = html_entity_decode($this->config->get('smtp_password')); // SMTP password
                        ...
                        $result = '';
                        $result .= $mail->HeaderLine("Organization" , "SITE"); 
                        $result .= $mail->HeaderLine("Content-Transfer-encoding" , "8bit");
                        $result .= $mail->HeaderLine("Message-ID" , "<".md5(uniqid(time()))."@{$this->config->get('smtp_hostname')}>");
                        ...
                        $result .= $mail->HeaderLine("X-AntiAbuse" , "Servername - {$this->config->get('smtp_hostname')}");
                        $result .= $mail->HeaderLine("X-AntiAbuse" , $mail->Sender);
                        
                        $mail->Subject  = sprintf($this->language->get('email_subject'));//
                        $mail->Body     = strip_tags($this->request->post['enquiry']);//
                        $mail->CharSet  = 'utf-8';
                        $mail->Send();
                  */      
