...
class ControllerPaymentBoletocefsigcb extends Controller {
	public function index() {
		...
		//$data['idboleto'] = $encryption->encrypt($this->session->data['order_id']);
                $data['idboleto'] = $this->encryption->encrypt($this->session->data['order_id']);
                
                
		$data['continue'] = $this->url->link('checkout/boleto_success');
	...
	public function confirm() {
		...
		$codigo_boleto = $this->session->data['order_id'];

		$comment  = $this->language->get('text_instruction') . "\n\n";
		$comment .= sprintf($this->language->get('text_linkboleto'), $codigo_boleto). "\n\n";
		$comment .= $this->language->get('text_payment');
		
		$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('boleto_cef_sigcb_order_status_id'), $comment);
            }
	...
	public function callback() {
		...
		$order_info = $this->model_checkout_order->getOrder($order_id);
                ...
                if($order_info){
                        ...
                        $dadosboleto["empresa_logo"] = $this->config->get("config_url")."image/".$this->config->get('boleto_cef_sigcb_logo');
